...
dir_gcm="/Users/marco/Documents/dati/CMIP5_CMIP6/tasmax/rcp85/"
listGCMs = file.ls <- list.files(path = dir_gcm,
                                 pattern = glob2rx("*_hist.nc4"))
...
for (ifile in 1:length(listGCMs)) {
  ...
  tx = c(prec_ctrl, prec_sim)
  
  save(tx, file = paste0(dir_gcm,gsub("_hist.nc4","",listGCMs[ifile]),'.RData'))
