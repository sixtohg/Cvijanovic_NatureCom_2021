...
dir_sim='/Users/marco/Documents/dati/ivana_ice_fires/'
...
files = list.files(pattern = "cam.2040y_ptt_baileys_cal.RData")
changes=vector()
changes_tasmax=vector()
changes_spi=vector()

for (ifile in 1:(length(files)/2)) {
  #namefile = files[ifile]
  
  nomefile=as.character(files[ifile])
  
  load(paste0('SPI4_',substr(nomefile,6,nchar(nomefile))))
  ...
  load(paste0('TX_',substr(nomefile,6,nchar(nomefile))))
  ...
  writeMat(file.path(dir_oss, paste0('low_ice/spi4_3_',ifile-1,'_nostd_',domain,'.mat')), spi4_3=spi4_3)
  writeMat(file.path(dir_oss, paste0('low_ice/tx_4_9_',ifile-1,'_nostd_',domain,'.mat')), tx_4_9=tx_4_9)
  
  
  # spi4_3=scale(aux)
  # tx_4_10=scale(aux_tx)
  # 
  # writeMat(file.path(dir_sim, paste0('spi5_10_',ifile-1,'_',domain,'.mat')), spi4_3=spi4_3)
  # writeMat(file.path(dir_sim, paste0('tx_4_9_',ifile-1,'_',domain,'.mat')), tx_4_10=tx_4_10)
  # 
...
boxplot(changes_spi,ylim=c(-1.2,0.8))
abline(h=0)
...
png(file = paste0(dir_out,"change_tsmax_lowicee.jpeg"), width = 500, height = 1000)
par(mar=c(10,6,4,1)+.1)
barplot(changes_tasmax,ylab="Degree", col= "black", las=2, border = 0, cex.lab=1, cex.axis=1, font=1,col.axis="black")
dev.off()

png(file = paste0(dir_out,"change_spi_lowice.jpeg"), width = 500, height = 1000)
par(mar=c(10,6,4,1)+.1)
barplot(changes_spi,ylab="Degree", col= "black", las=2, border = 0, cex.lab=1, cex.axis=1, font=1,col.axis="black")
dev.off()
