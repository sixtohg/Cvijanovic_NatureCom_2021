...
for ifile=1:length(listGCMs_SPI)
    ...
    pred = B1(1)  + B1(2) * scale_1981_2000(spi4_3,years_sim) + B1(4) * mean(years) + B1(5) * mean(years.*years);
    ...
    for ib=1:1000
        k=k+1;
        pred = bootb1(ib,1)  + bootb1(ib,2) * scale_1981_2000(spi4_3,years_sim) + bootb1(ib,4) * mean(years') + bootb1(ib,5) * mean(years.*years)';
        ctrl_ens=nanmean(exp(pred(1:20)));
        ice_ens=nanmean(exp(pred(21:40)));
