...
dir_gcm_pr="/Users/marco/Documents/dati/CMIP5_CMIP6/pr/ssp585/"
dir_gcm_tasmax="/Users/marco/Documents/dati/CMIP5_CMIP6/tasmax/ssp585/"
listGCMs_pr = list.files(path=dir_gcm_pr,pattern="^pr_(.*)_spi4.RData$")
listGCMs_tasmax = list.files(path=dir_gcm_tasmax,pattern="^tasmax_(.*).RData$")
...
changes_spi=vector()
years_gcm=1981:2020
pred_gcm=matrix(NA, nrow = length(listGCMs_pr), ncol =length(years_gcm) )

for (ifile in 1:length(listGCMs_pr)) {
  load(paste0(dir_gcm_tasmax,listGCMs_tasmax[ifile]))
  load(paste0(dir_gcm_pr,listGCMs_pr[ifile]))
  ...
  writeMat(paste0(dir_gcm_tasmax,gsub(".RData","",listGCMs_tasmax[ifile]),'.mat'),tx_4_9=tx_4_9)
  writeMat(paste0(dir_gcm_pr,gsub(".RData","",listGCMs_pr[ifile]),'.mat'),spi4_3=spi4_3)
  writeMat(file.path(dir_oss, paste0('CMIP6/spi4_3_',ifile-1,'_nostd_',domain,'.mat')), spi4_3=spi4_3)
  writeMat(file.path(dir_oss, paste0('CMIP6/tx_4_9_',ifile-1,'_nostd_',domain,'.mat')), tx_4_9=tx_4_9)
