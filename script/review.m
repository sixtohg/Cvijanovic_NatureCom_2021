...
%% Changes in TSMAX:
dTx=[-1:0.1:4];cTx=repmat(NaN,length(Y1),length(dTx));
for i=1:length(dTx)
	auxX=Xorig1;auxX(31:50,3)=Xorig1(31:50,3)+dTx(i);
	cTx(:,i) = (auxX*B1);
end
auxTx = 100*(mean(exp(cTx(31:50,:))) - mean(exp(Y1(31:50))))./mean(exp(Y1(31:50))); 
figure,plot(dTx,auxTx,'-k','linewidth',2),xlabel('Change in TSMAX (ºC)'),ylabel('Relative Change in BA w.r.t. 1981-2000 (%)'), grid on

%% Changes in SPI:
dTx=[-1:0.1:1];cTx=repmat(NaN,length(Y1),length(dTx));
for i=1:length(dTx)
	auxX=Xorig1;auxX(31:50,2)=Xorig1(31:50,2)+dTx(i);
	cTx(:,i) = (auxX*B1);
end
auxTx = 100*(mean(exp(cTx(31:50,:))) - mean(exp(Y1(31:50))))./mean(exp(Y1(31:50))); 
figure,plot(dTx,auxTx,'-k','linewidth',2),xlabel('Change in SPI (-)'),ylabel('Relative Change in BA w.r.t. 1981-2000 (%)'), grid on
...
auxP=exp(B1(2)*refSPI+ B1(3)*refTmax);
...
figure(5),subplot(2,2,1),bar(clim_cmip5(:,1)','grouped','g'), set(gca,'xlim',[0 23],'ylim',[0 0.6])
hold on,plot([0 23],[nanmean(exp(yor(31:50))) nanmean(exp(yor(31:50)))],'--k'),for i=1:size(clim_cmip5,1),plot([i i],clim_cmip5(i,2:3),'-k'),end
figure(5),subplot(2,2,2),bar(clim_cmip6(find(~isnan(clim_cmip6(:,1))),1)','grouped','r'), set(gca,'xlim',[0 23],'ylim',[0 0.6])
hold on,plot([0 23],[nanmean(exp(yor(31:50))) nanmean(exp(yor(31:50)))],'--k'),for i=1:size(clim_cmip6,1),plot([i i],clim_cmip6(i,2:3),'-k'),end
figure(5),subplot(2,2,3),bar(clim_lowice(:,1)','grouped','b'), set(gca,'xlim',[0 23],'ylim',[0 0.6])
hold on,plot([0 23],[nanmean(exp(yor(31:50))) nanmean(exp(yor(31:50)))],'--k'),for i=1:size(clim_lowice,1),plot([i i],clim_lowice(i,2:3),'-k'),end
figure(5),subplot(2,2,4),bar(clim_EC(:,1)','grouped','c'), set(gca,'xlim',[0 23],'ylim',[0 0.6])
hold on,plot([0 23],[nanmean(exp(yor(31:50))) nanmean(exp(yor(31:50)))],'--k'),for i=1:size(clim_EC,1),plot([i i],clim_EC(i,2:3),'-k'),end
...
caxis([-60 780]);colormap(fall);h=colorbar;hold on
legend({['Low-Ice (' num2str(median(changes_lowice(find(~isnan(changes_lowice(:,1))),1)),'%5.1f') ')'],['Low-Ice EC-Earth (' num2str(median(changes_EC(find(~isnan(changes_EC(:,1))),1)),'%5.1f') ')'],['CMIP5 (' num2str(median(changes_cmip5(find(~isnan(changes_cmip5(:,1))),1)),'%5.1f') ')'],['CMIP6 (' num2str(median(changes_cmip6(find(~isnan(changes_cmip6(:,1))),1)),'%5.1f') ')']},'Location','Best')
%% grid on
set(h, 'YTick', [-40:40:760])
set(h,'YTickLabel',[-40:40:760])
title(h,'Changes in BA','FontWeight','bold')
