...
for (im in 1:9) {
  ...
  for (istop in 1:length(tmp_stop)) {
    ...
    for (iyear in 1:length(years)) {
      ...
      tx[iyear] = mean(nclimgrid[11, i1:i2], na.rm = TRUE)
    }
    ...
    for (im_spi in 1:9) {
      for (isc in 1:length(iscC)) {
        ...
        for (iy in 1:length(years)) {
          ...
          pre123[iy] = fit123$coefficients[1] + fit123$coefficients[2] * mydata_test$x1 + fit123$coefficients[3] * mydata_test$x2 + fit123$coefficients[4] * mydata_test$x3
...
mydata_reg = data.frame(
  "y" = log(BAS/1000),
  "x1" = years_model,
  "x2" = detrend(scale_1981_2000(tx,years_model)),
  "x3" = detrend(scale_1981_2000(SPI_aux,years_model))
)
